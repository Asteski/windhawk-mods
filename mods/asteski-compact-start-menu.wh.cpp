...
- hideSearchBar: false
  ...
  $description: Hide the Start menu search bar.
...
* **Optional search bar hiding:** Can hide the Start menu search bar.
...
void LoadSettings() {
    ...
    g_settings.hideSearchBar = Wh_GetIntSetting(L"hideSearchBar") != 0;
...
void HideStartMenuElement(wux::FrameworkElement const& element) {
    ...
    if (IsGroupHeaderElement(element) ||
        (g_settings.hideSearchBar && IsSearchBarElement(element)) ||
        (g_settings.hidePinnedSection &&
